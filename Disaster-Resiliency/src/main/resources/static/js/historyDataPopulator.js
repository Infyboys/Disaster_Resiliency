...
function populateDisasters(){
	var disasterList = $("#disasterList")[0].innerHTML;
	if(undefined != disasterList){
        disasterList = jQuery.parseJSON(disasterList);
        for (var i = 0; i < disasterList.length; i++) {
			$("#disasters").append ("<option value='"+ disasterList[i].name +"'>");
        }
	}
}
...
function addBubble(){
	ui.addBubble(new H.ui.InfoBubble({lat:parseFloat(locationList[index].latitude), lng:parseFloat(locationList[index].longitude)}, {
      content: "<a target='_blank' href='https://" + locationList[index].articleLink + "'>"+ locationList[index].articleLink +"</a><br>" + locationList[index].keyPoints + "<br>" + locationList[index].date 
    }));
}
...
function getDomMarker(index) {
  ...
  innerElement.style.marginLeft = '-10px';
	...
	outerElement.setAttribute("title", locationList[index].keyPoints);
	
  outerElement.appendChild(innerElement);

  // Add text to the DOM element
  innerElement.innerHTML = locationList[index].disaster.charAt(0);
